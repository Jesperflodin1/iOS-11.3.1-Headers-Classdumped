...
typedef struct aligned_storage_imp<8, 8> {
	data_t data_;
	char buf[8];
	a8) align_;
} aligned_storage_imp<8, 8>;

typedef struct VtValue {
	aligned_storage_imp<8, 8> _storage;
	TfPointerAndBits<const VtValue::_TypeInfo> _info;
} VtValue;

typedef struct VtValuealigned_storage_imp<8, 8>(data_t[8c]a8)TfPointerAndBits<const VtValue::_TypeInfo>_TypeInfo* tValueRef;
