/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, August 22, 2018 at 12:24:19 AM Central European Summer Time
* Operating System: Version 11.3.1 (Build 15E302)
* Image Source: /System/Library/PrivateFrameworks/NeutrinoCore.framework/NeutrinoCore
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <NeutrinoCore/NeutrinoCore-Structs.h>
@interface NUImageLayout : NSObject {

	SCD_Struct_NU5 _imageSize;

}

@property (nonatomic,readonly) SCD_Struct_NU5 imageSize;              //@synthesize imageSize=_imageSize - In the implementation block
@property (nonatomic,readonly) long long tileCount; 
+(id)contiguousLayoutForImageSize:(SCD_Struct_NU5)arg1 ;
+(id)tiledLayoutForImageSize:(SCD_Struct_NU5)arg1 tileSize:(SCD_Struct_NU5)arg2 ;
+(id)overlappingTiledLayoutForImageSize:(SCD_Struct_NU5)arg1 tileSize:(SCD_Struct_NU5)arg2 borderSize:(SCD_Struct_NU5)arg3 ;
+(id)stripLayoutForImageSize:(SCD_Struct_NU5)arg1 stripHeight:(long long)arg2 ;
-(BOOL)isEqual:(id)arg1 ;
-(SCD_Struct_NU5)tileSize;
-(SCD_Struct_NU5)imageSize;
-(void)enumerateTilesForReadingInRect:(SCD_Struct_NU6)arg1 withBlock:(/*^block*/id)arg2 ;
-(id)tileInfoAtIndex:(long long)arg1 ;
-(BOOL)isEqualToLayout:(id)arg1 ;
-(SCD_Struct_NU6)frameRectForTileAtIndex:(long long)arg1 ;
-(SCD_Struct_NU6)contentRectForTileAtIndex:(long long)arg1 ;
-(long long)tileIndexAtPoint:(SCD_Struct_NU5)arg1 ;
-(void)enumerateTilesForWritingInRect:(SCD_Struct_NU6)arg1 withBlock:(/*^block*/id)arg2 ;
-(SCD_Struct_NU5)borderSize;
-(SCD_Struct_NU5)tileCounts;
-(id)initWithImageSize:(SCD_Struct_NU5)arg1 ;
-(long long)tileCount;
@end

