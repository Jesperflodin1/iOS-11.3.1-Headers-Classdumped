...
@class NSObject, VCCallInfo, VCConnectionManager, VCMediaNegotiator, NSMutableArray, VCAudioPayload, VCImageAttributeRules, VCCapabilities, GKRingBuffer, NSDictionary, NSMutableDictionary, NSArray, TimingCollection, NSString, VCCallLinkCongestionDetector, NSData, VCSessionMessaging, VCBitrateArbiter, VCAudioTransmitter, VCCaptionsReceiver, VideoAttributes, VCWCMClient, WRMClient, VCSecureDataChannel, NSNumber, AVTelephonyInterface, VCTransportSession, AVCRateController, VCRateControlMediaController;

@interface VCCallSession : NSObject <VCSecureDataChannelDelegate, AVTelephonyInterfaceDelegate, VCCaptionsReceiverDelegate, VCTransportSessionLegacyDelegate, AVCRateControllerDelegate, VCAudioIOSink> {
...
-(void)sendWRMStatusUpdate:(const SCD_Struct_VC71*)arg1 ;
...
-(BOOL)onCaptureFrame:(opaqueCMSampleBufferRef)arg1 audioTS:(unsigned)arg2 audioHT:(double)arg3 videoHT:(SCD_Struct_VC33)arg4 droppedFrames:(int)arg5 cameraBits:(unsigned char)arg6 ;
-(void)processBlackFrame:(opaqueCMSampleBufferRef)arg1 ;
-(void)updateVideoQualityNotification:(double)arg1 ;
-(void)onPlayVideo:(CVBufferRef)arg1 frameTime:(SCD_Struct_VC33)arg2 cameraStatusBits:(unsigned char)arg3 ;
