/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, August 22, 2018 at 12:27:17 AM Central European Summer Time
* Operating System: Version 11.3.1 (Build 15E302)
* Image Source: /System/Library/PrivateFrameworks/Silex.framework/Silex
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@protocol SXLayouterDelegate;
@interface SXLayouterFactory : NSObject {

	id<SXLayouterDelegate> _layouterDelegate;

}

@property (nonatomic,__weak,readonly) id<SXLayouterDelegate> layouterDelegate;              //@synthesize layouterDelegate=_layouterDelegate - In the implementation block
-(id)initWithDelegate:(id)arg1 ;
-(id)layouterForContainerComponentBlueprint:(id)arg1 ;
-(id<SXLayouterDelegate>)layouterDelegate;
@end

