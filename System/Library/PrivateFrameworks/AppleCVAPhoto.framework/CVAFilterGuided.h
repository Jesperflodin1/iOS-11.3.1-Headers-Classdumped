/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, August 22, 2018 at 12:13:40 AM Central European Summer Time
* Operating System: Version 11.3.1 (Build 15E302)
* Image Source: /System/Library/PrivateFrameworks/AppleCVAPhoto.framework/AppleCVAPhoto
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@protocol MTLDeviceSPI, MTLLibrary, MTLTexture, MTLComputePipelineState, MTLBuffer;
#import <AppleCVAPhoto/AppleCVAPhoto-Structs.h>
@class CVAFilterBox, CVAFilterHybridResampling, CVAFilterInfimumConvolution, NSString;

@interface CVAFilterGuided : NSObject {

	id<MTLDeviceSPI> _device;
	id<MTLLibrary> _library;
	CVAFilterBox* _boxFilter;
	CVAFilterHybridResampling* _hybridResampler;
	id<MTLTexture> _gfSrcWithGuide;
	id<MTLTexture> _gfMeanSrcWithGuide;
	id<MTLTexture> _gfSrcGuide;
	id<MTLTexture> _gfMeanSrcGuide;
	id<MTLTexture> _gfRRRGRB;
	id<MTLTexture> _gfGGGBBB;
	id<MTLTexture> _gfMeanRRRGRB;
	id<MTLTexture> _gfMeanGGGBBB;
	id<MTLTexture> _gfTmp;
	id<MTLTexture> _gfTmp2;
	id<MTLTexture> _gfMeanWeight;
	id<MTLTexture> _gfMeanNormalizationFactor;
	id<MTLTexture> _convolutedCoeff;
	id<MTLTexture> _upscaledCoeff;
	id<MTLTexture> _filteredAlphaTexture;
	id<MTLTexture> _smoothedAlphaTexture;
	id<MTLTexture> _laplacianLimitedAlphaTexture;
	id<MTLTexture> _alphaTexture;
	id<MTLTexture> _hybridUpscalingIntermediateTexture;
	id<MTLTexture> _hybridUpscalingIntermediateR32Texture;
	id<MTLTexture> _srcTexture;
	id<MTLTexture> _lowResMatte;
	id<MTLTexture> _lowResLaplacianLimitedMatte;
	id<MTLComputePipelineState> _guidedFilterApplyRGBAKernel;
	id<MTLComputePipelineState> _guidedFilterInvertKernel;
	id<MTLComputePipelineState> _guidedFilterMultiplyKernel;
	id<MTLComputePipelineState> _guidedFilterEmbedGuideKernel;
	id<MTLComputePipelineState> _guidedFilterPremultiplyKernel;
	id<MTLComputePipelineState> _guidedFilterSecondOrderProductsKernel;
	id<MTLComputePipelineState> _guidedFilterRegressionKernel;
	id<MTLBuffer> _regressionConfig;
	id<MTLComputePipelineState> _postProcessAlphaKernel;
	CVAFilterInfimumConvolution* _infConvolution;
	int _smallGuidedFilterKernelSize;
	NSString* _label;

}

@property (readonly) NSString * label;              //@synthesize label=_label - In the implementation block
-(NSString *)label;
-(id)initWithDevice:(id)arg1 library:(id)arg2 commandQueue:(id)arg3 textureSize:(SCD_Struct_CV25)arg4 alphaSize:(SCD_Struct_CV25)arg5 kernelSize:(int)arg6 ;
-(void)encodeRegressionToCommandBuffer:(id)arg1 inSourceTexture:(id)arg2 inGuidanceTexture:(id)arg3 inWeightTexture:(id)arg4 outCoeffTexture:(id)arg5 epsilon:(float)arg6 ;
-(void)encodeReconstructToCommandBuffer:(id)arg1 inGuidancePixelBuffer:(CVBufferRef)arg2 inCoeffTexture:(id)arg3 outAlphaPixelBuffer:(CVBufferRef)arg4 alphaMaxLaplacian:(float)arg5 gravity:(const array<float, 3>*)arg6 isBgColorLutBlack:(BOOL)arg7 gammaExponent:(float)arg8 enableInfConvolution:(BOOL)arg9 ;
-(void)encodeReconstructToCommandBuffer:(id)arg1 inGuidancePixelBuffer:(CVBufferRef)arg2 inCoeffTexture:(id)arg3 outFilteredTexture:(id)arg4 ;
-(void)initSourceTexture:(CVBufferRef)arg1 ;
-(void)encodeFilterApplyToBuffer:(id)arg1 coeff:(id)arg2 source:(id)arg3 destination:(id)arg4 ;
-(void)encodeSmoothAlphaToCommandBuffer:(id)arg1 alpha:(id)arg2 ;
-(void)encodePostProcessAlphaToCommandBuffer:(id)arg1 source:(id)arg2 destination:(id)arg3 alphaMaxLaplacian:(float)arg4 infConvOrientation:(float)arg5 infConvMajorRadius:(float)arg6 infConvMinorRadius:(float)arg7 isBgColorLutBlack:(BOOL)arg8 gammaExponent:(float)arg9 enableInfConvolution:(BOOL)arg10 ;
-(void)encodeHybridUpSamplingToCommandBuffer:(id)arg1 source:(id)arg2 destination:(id)arg3 ;
@end

